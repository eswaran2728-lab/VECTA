...
import { requireRole, ENFORCEMENT_SEARCH_ROLES } from "@/lib/auth";
import { searchFlightAttendance } from "@/lib/search/enforcementSearch";
...
export default async function EnforcementSearchPage({
  ...
  searchParams: { flight?: string; date?: string };
}) {
  ...
  return (
    <main className="min-h-screen pb-32">
      <AppHeader profile={profile} title="Enforcement Search" backHref="/dashboard" />
          ...
          <div className="space-y-2">
            ...
            {search.results.map((r) => (
              <Link
                ...
                href={`/reports/view/${r.report_type}/${r.report_id}`}

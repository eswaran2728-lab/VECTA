...
import { getZonesForStation, getZoneById } from "@/lib/duty/zone-queries";
import { upsertZone, toggleZoneActive } from "@/lib/duty/zone-actions";
import { AppHeader } from "@/components/layout/AppHeader";
import ZoneEditorLoader from "@/components/duty/ZoneEditorLoader";

export default async function AdminZonesPage({
  searchParams,
}: {
  searchParams: { station?: string; edit?: string; error?: string };
}) {
  ...
  return (
    <main className="min-h-screen pb-16">
      <AppHeader profile={profile} title="Duty Zones" backHref="/admin/users" />
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
            ...
            {zones.map((z) => (
              <div key={z.id} className="flex items-center justify-between py-2.5 gap-2">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link href={`/admin/zones?station=${encodeURIComponent(station)}&edit=${z.id}`} className="btn-quiet">
                    Edit
                  </Link>
        ...
        <div className="card p-4 space-y-3">
          ...
          <form action={upsertZone} className="space-y-3">
            ...
            {editing && (
              <Link href={`/admin/zones?station=${encodeURIComponent(station)}`} className="btn-quiet w-full block text-center">
                Cancel edit
              </Link>
            )}
          </form>

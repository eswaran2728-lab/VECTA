...
export async function updateSheetSyncConfig(formData: FormData) {
  ...
  revalidatePath("/avsec/admin/sheet-sync");
  if (error) redirect(`/avsec/admin/sheet-sync?error=${encodeURIComponent(error.message)}`);
}
...
export async function triggerSheetSyncNow() {
  ...
  if (!baseUrl || !cfg?.webhook_secret) {
    redirect("/avsec/admin/sheet-sync?error=" + encodeURIComponent("Set and save a webhook secret first."));
  }
  ...
  if (errorMessage) {
    redirect("/avsec/admin/sheet-sync?error=" + encodeURIComponent(errorMessage));
  }
